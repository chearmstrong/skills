...
| [`agentic-systems-design`](skills/agentic-systems-design)                 | Use when designing or reviewing an LLM-based agentic system, agent workflow, autonomy model, deterministic flow, single-agent system, multi-agent system, or agent architecture. Helps select the lowest sufficient autonomy, map reasoning, orchestration, tool, state, memory, identity, approval, observability, evaluation, and cost boundaries, and produce evidence-backed designs or review findings. |
| [`architecture-compliance-check`](skills/architecture-compliance-check)       | Verify architecture and implementation against documented patterns, project rules, and authoritative evidence. Use when reviewing code, architecture spikes, reusable assets, delegated-workflow boundaries, or design documents; separates verified facts from proposals and checks contracts, assumptions, and cross-document consistency.                                                                                                                   |
